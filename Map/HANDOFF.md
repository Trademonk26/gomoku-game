# HANDOFF.md — 작업 인수인계 가이드 (2026-07-06 기준)
...
**검증 상태 (`ce80096`에서 전부 통과)**: pytest 6 · vitest 37(계약 테스트) · `npm run build` · Playwright E2E 콘솔 오류 0건.

**라이브 지표 8개**: P1·P2(OSM 변전소 근접/밀도), P6(OSM 발전설비), W5(ERA5 냉방도일), N1(육양국), N2(서울 IX 지연 산식), H3(지진구역계수), A1(유치 의지 큐레이션). L(토지)·S(부지) 축은 결측 — 가중치 재분배 + 커버리지 %로 정직 표시 중.
...
| 순서 | 지표 | 소스 | 비고 |
|---|---|---|---|
| 1 | S1 미분양·S6 분양공고 산업용지 | 국토부 산업시설용지 분양현황(15015589) / ILIS industryland.or.kr 통계 다운로드 | 분기 XLS — 지역별 현황을 시군구로 매핑. **도시 편중 교정 효과가 가장 큰 축** |
| 2 | S2 공급 파이프라인 | ILIS 조성중·계획 산업단지 | S1과 같은 소스 계열 |
| 3 | L3 평지 비율 | Copernicus GLO-30 DEM(무키) | 경사 5°·표고 200m 기준, 시군구 존율 계산 — 연산량 주의(타일 캐시) |
| 4 | P12 송변전 건설사업 단계 | 한전 정보공개 게시판(BLUEPRINT §4.2 URL) | 44페이지 크롤 → 사업명에서 지역 추출(수동 검수 파일 두기) |
| 5 | P11 공급가능 변전소 수 | data.go.kr 15128065 | 읍면동 단위 → 시군구 집계. 2024.5 스냅샷 캐비앳 유지. 다운로드 로그인 필요 여부 확인 |

### B. API 키 필요 (사용자에게 발급 요청)
| 키 | 발급처 | 잠금 해제 |
|---|---|---|
| V-World | vworld.kr | L1 용도지역 비율, L2 규제 중첩(게이트 레이어) |
| 공공데이터포털 | data.go.kr | L4 개별공시지가 중앙값 |
